...
namespace Soda
{
    ...
    struct DiagnosticPosition
    {
        ...
        DiagnosticPosition(size_t position = 0, size_t line = size_t(-1),
    ...
    public:
        ...
        Diagnostic(
            DiagnosticCategory cat, DiagnosticLocation loc, std::string msg)
            : category(cat)
        ...
        friend std::ostream &operator<<(
            std::ostream &os, const Diagnostic &diag)
        {
            std::string cat;
    ...
    public:
        template < class... Args >
        void log(
            DiagnosticCategory cat, DiagnosticLocation loc, Args &&... args)
        {
            std::stringstream ss;
        ...
        {
            log(cat, DiagnosticLocation(token.file, token.start, token.end),
    ...
    private:
        ...
        void format(
            std::ostream &os, const char *s, const T &value, Args &&... args)
        {
            while (*s) {
                if (*s == '%') {
                    if (*(s + 1) == '%')
