...
namespace Soda
{
    ...
    struct SourceFile::Private
    {
        ...
        {
#ifdef SODA_WIN32
            hFile = CreateFile(fn.c_str(), GENERIC_READ, 0, nullptr,
            ...
            if (dwSize == INVALID_FILE_SIZE)
                throw std::runtime_error("invalid file size of file '" + fn
                    + "' found (error code " + std::to_string(GetLastError())
                    + ")");
            else if (dwSize == 0)
                throw std::runtime_error(
                    "cannot open file '" + fn + "' of zero bytes");
            hMapFile = CreateFileMapping(
                hFile, nullptr, PAGE_READONLY, 0, dwSize, nullptr);
            if (hMapFile == NULL)
                throw std::runtime_error("failed to memory map file '" + fn
                    + "' (error code " + std::to_string(GetLastError()) + ")");
            mapData = MapViewOfFile(hMapFile, FILE_MAP_READ, 0, 0, 0);
            if (mapData == NULL)
                throw std::runtime_error(
                    "failed to create view of memory mapped file '" + fn
                    + "' (error code " + std::to_string(GetLastError()) + ")");
#else
            fd = ::open(fn.c_str(), O_RDONLY);
            if (fd < 0)
                throw std::runtime_error("failed to open file '" + fn + "': "
                    + strerror(errno) + " (error code " + std::to_string(errno)
                    + ")");
            struct ::stat sb;
            if (::stat(fn.c_str(), &sb) != 0)
                throw std::runtime_error("failed to stat file '" + fn + "': "
                    + strerror(errno) + " (error code " + std::to_string(errno)
                    + ")");
            dwSize = sb.st_size;
            mapData = ::mmap(nullptr, dwSize, PROT_READ, MAP_PRIVATE, fd, 0);
            if (mapData == MAP_FAILED)
                throw std::runtime_error("failed to map file '" + fn + "': "
                    + strerror(errno) + " (error code " + std::to_string(errno)
                    + ")");
#endif
        }
